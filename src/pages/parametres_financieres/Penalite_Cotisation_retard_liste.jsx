...
export default function Penalite_Cotisation_retard_liste() {
  ...
  const fetchPenalite = useCallback(async () => {
    try {
      setLoading(true);
      const baseurl = `/parametre/cotisationretard/fetch? `;
      var url = baseurl;
    ...
    <>
      ...
      <div className="px-4 py-3 main_content">
        <div className="d-flex align-items-center justify-content-between">
          <h1 className="mb-3"> Taux Pénalité cotisation retard</h1>
          {/* <Button
                    ...
                    const items = [

                      {
                        template: (deleteItem, options) => {
                          return (
                            <Link
                            to={`/retard/update/${encodeId(inViewMenuItem?.ID_COTISATION_RETARD)}`}
