...
export default function Notification_log_list_page() {
    ...
    const deleteItems = async (itemsIds) => {
        try {
            ...
            dispacth(
                setToastAction({
                    severity: "success",
                    summary: " Notification supprimé",
                    detail: "L ' Notification a été supprimé avec succès",
                    life: 3000,
                })
            );
            fetchEmploye();
    ...
    const fetchEmploye = useCallback(async () => {
        try {
            setLoading(true);
            const baseurl = `/gerers/employe/fetch?&rows=${lazyState.rows}&`;
            var url = baseurl;
            ...
            // console.log ('---------------',res)
            setNotification(res.result.data);
    ...
    useEffect(() => {
        ...
        dispacth(setBreadCrumbItemsAction([administration_routes_items.employe]));
        return () => {
        ...
        <>
            ...
            <div className="px-4 py-3 main_content">
                <div className="d-flex align-items-center justify-content-between">
                    <h1 className="mb-3">Notification</h1>
                    <Button
                        label="Nouveau"
                        icon="pi pi-plus"
                        size="small"
                        onClick={() => {
                            navigate("/employe/new");
                        }}
                    />
                </div>
                ...
                <div className="content">
                    <div className="shadow rounded mt-3 pr-1 bg-white">
                        ...
                        >
                            ...
                            />
                            <Column field="LIBELLE" header="Fonction " sortable body={item => item?.types_de_fonction?.LIBELLE || '-'} />
                                    ...
                                    const items = [

                                        {
                                            template: (deleteItem, options) => {
                                                return (
                                                    <Link
                                                        to={`/employe/edit/${encodeId(inViewMenuItem?.ID_EMPLOYE)}`}
