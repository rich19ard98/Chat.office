...
*/
export default function Compte_Resultant_liste_page() {
  const [selectedCity, setSelectedCity] = useState(null);
  const [date, setDate] = useState(null);
  const [isVisible, setIsVisible] = useState(false);
  const [selectAll, setSelectAll] = useState(false);
  const [loading, setLoading] = useState(true);
  const [TotalRecordscharge, setTotalRecordscharge] = useState(0);
  const [TotalRecordsproduits, setTotalRecordsproduits] = useState(0);
  ...
  const fetchClassecomptable = useCallback(async () => {
    try {
      setLoading(true);
      const baseurl = `/plancomptable/ecriturecomptable/fetchCompteResult?rows=10000000000&`;
      var url = baseurl;
      for (let key in lazyState) {
        const value = lazyState[key];
        if (value) {
      ...
      setCharges(res.result.detailsCharge)
      setProduits(res.result.detailsProduit)


      setTotalRecordscharge(res.result.totalRecordscharge);
      setTotalRecordsproduits(res.result.totalRecordsproduit);
  ...
  useEffect(() => {
    document.title = "Liste des comptes résultants"
  ...
  const exportPdf = () => {
    ...
    doc.addImage(entete, "JPEG", 0, 2, 50, 20);

    // Titre sous l'image
    doc.setFontSize(13);
    doc.setFont("helvetica", "bold");
    doc.text(`Balance des comptes du le : ${moment(Date()).format("DD/MM/YYYY")}`, 100, 30, { align: "center" });

    const columns = [
      { title: "#", dataKey: "NUMERO" },
      { title: "Code", dataKey: "nom_compte" },
      { title: "Libelle", dataKey: "type_operation" },
      { title: "Montant", dataKey: "montant_total" },
    ];
    ...
    doc.autoTable({
      ...
      body: produit.map((item, index) => [
        index + 1,
        item.comptedebit?.CODE ? item.comptedebit?.CODE : "-",
        item?.type_operation ? item?.type_operation : "-",
        item.montant_total ? `${parseFloat(item.montant_total).toLocaleString('fr-FR').replace(/\s/g, ' ')} Fbu` : "0 Fbu",
      ]),
    ...
    doc.setFontSize(13);
    doc.setFont("helvetica", "bold");
    doc.text("Produit", 5, finalY - 20);

    // Définir les colonnes pour le tableau Charges
    doc.setFontSize(13);
    doc.setFont("helvetica", "bold");
    doc.text("Charges", 5, finalY + 20);

    // Générer le deuxième tableau
    doc.autoTable({
      startY: finalY + 20 + 10,
      head: [columns.map(col => col.title)],
      body: charges.map((item, index) => [
        index + 1,
        item.comptedebit?.CODE ? item.comptedebit?.CODE : "-",
        item?.type_operation ? item?.type_operation : "-",
        item.montant_total ? `${parseFloat(item.montant_total).toLocaleString('fr-FR').replace(/\s/g, ' ')} Fbu` : "0 Fbu",
    ...
    doc.setFontSize(12);
    doc.setFont("helvetica", "normal");
    doc.text(`${totalProduit}`, 5, totalY + 10);
    doc.text(`${totalCharges}`, 100, totalY + 10);
    doc.text(`${totalProduit - totalCharges} Fbu`, 180, totalY + 10, { align: "center" });
        ...
        <>
          <div className="px-4 py-3 main_content">
            <div className="d-flex align-items-center justify-content-between">
              <h1 className="mb-3">Comptes des resultants</h1>
            ...
            <div className="shadow my-2 bg-white p-3 rounded d-flex align-items-center justify-content-between">
              ...
              {plancomptablePermission && plancomptablePermission.CAN_WRITE ?
                <div className="selection-actions d-flex align-items-center ml-3">
                  <Button
                    disabled={produit.length == 0 && charges.length == 0}
                    className=" mt-3 ml-3 button-mobile "
                    size="small"
                    type="submit"
            ...
            <div className="content">
              ...
              <div className="shadow rounded mst-3 pr-1 bg-white">
                <DataTable
                  lazy
                  value={produit}
                  ...
                  rowsPerPageOptions={[5, 10, 25, 50,100,200,500,1000]}
                   ...
                   currentPageReportTemplate={` {first} - {last} dans ${TotalRecordsproduits} éléments`}
                  emptyMessage="Aucun Classe comptable trouvé"
                  // paginatorLeft={paginatorLeft}
                  // paginatorRight={paginatorRight}
                  first={lazyState.first}
                  rows={lazyState.rows}
                  totalRecords={TotalRecordsproduits}
                  onPage={onPage}
                  onSort={onSort}
                ...
                >
                  ...
                  <Column
                    ...
                    body={(item) => {
                      return (
                        <span>{item.comptecredit?.CODE}</span>
                      );
                    }}
                  ...
                  <Column
                    ...
                    body={(item) => {
                      return (
                        <span>{item.montant_total ? `${parseFloat(item.montant_total).toLocaleString('fr-FR')} Fbu` : '-'}</span>
                      );
                    }}
                  ...
                  <Column
                    ...
                    body={(item) => {
                      return (
                        <span>{item.type_operation ? item.type_operation : '-'}</span>
                      );
                    }}
                  />
            ...
            <div className="content">
              ...
              <div className="shadow rounded mt-s3 pr-1 bg-white">
                <DataTable
                  lazy
                  value={charges}
                  tableStyle={{ minWidth: "50rem" }}
                  ...
                  rowsPerPageOptions={[5, 10, 25, 50,100,200,500,1000]}
                   paginatorTemplate="FirstPageLink PrevPageLink PageLinks NextPageLink LastPageLink CurrentPageReport RowsPerPageDropdown"
                   currentPageReportTemplate={` {first} - {last} dans ${TotalRecordscharge} éléments`}
                  emptyMessage="Aucun Classe comptable trouvé"
                  // paginatorLeft={paginatorLeft}
                  // paginatorRight={paginatorRight}
                  first={lazyState.first}
                  rows={lazyState.rows}
                  totalRecords={TotalRecordscharge}
                  onPage={onPage}
                  onSort={onSort}
                  // onPage={onPage}
                  // onSort={onSort}
                ...
                >
                  ...
                  <Column
                    ...
                    body={(item) => {
                      return (
                        <span>{item.comptedebit.CODE}</span>
                      );
                    }}
                  />
            ...
            <div className="content">
              <div className="d-flex align-items-center justify-content-between">
                <h2 className="mb-3">Total compte resultant</h2>
              </div>
              <div className="shadow rounded mt-3 pr-1 bg-white">
                <DataTable
                  value={[totalProduit]} // Pas de données à afficher ici
                  tableStyle={{ minWidth: "50rem" }}
                  className="w-100"
                  showGridlines
                >
                  <Column field="TotalDebit" header="Total Débit" body={() => `${totalProduit} Fbu`} />
                  <Column field="TotalCredit" header="Total Crédit" body={() => `${totalCharges} Fbu`} />
                  <Column field="TotalSolde" header="Total Solde" body={() => `${totalProduit - totalCharges} Fbu`} />
                </DataTable>
              </div>
            </div>
          </div>
