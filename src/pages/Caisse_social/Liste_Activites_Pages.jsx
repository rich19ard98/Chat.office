...
export default function Liste_Activites_Pages() {
    ...
    const { ID_ACTIVITE_CAISSE : encodedId } = useParams();
    const ID_ACTIVITE_CAISSE  = decodeId(encodedId); // Décoder pour obtenir l'ID réel
    ...
    const fetchCotisation = useCallback(async () => {
        try {
            setLoading(true);
            const baseurl = `/cotisation/UtilisationCaisseSocial/fetch?rows=100000000000000&`;
            ...
            if (selectedMembre?.code) {
              ...
              if (ID_ACTIVITE_CAISSE) {
                url += `ID_ACTIVITE_CAISSE=${ID_ACTIVITE_CAISSE}&`;
            }
    ...
    }, [lazyState, startDate,ID_ACTIVITE_CAISSE, endDate, dates, selectedMembre]);

    useEffect(() => {
        fetchCotisation();
    }, [lazyState, startDate, endDate,ID_ACTIVITE_CAISSE, dates, selectedMembre]);
  ...
  */
    const exportExcelFrais = (frais) => {
        ...
        const titre = [["Liste des Activites"]];
        const headers = [
            "#",
            "Auteur",
            "Membre",
        ...
        const data = frais.map((item, index) => ([
            ...
            item?.DESCRIPTION || "-",
            item.typeoperation?.NOM_OPERATION || "-",
        ...
        const filename = `Activites${moment().format("YYYYMMDD_HHmmss")}.xlsx`;
    ...
    const exportPdfFrais = (frais) => {
        ...
        doc.setFontSize(13);
        doc.text("Liste des Activites", 80, 30);
        doc.setFontSize(13);
        ...
        const rows = frais.map((item, index) => ({
            ...
            OPERATION: item.typeoperation?.NOM_OPERATION || "-",
        ...
        const filename = `Listes_activites${moment().format("YYYYMMDD_HHmmss")}.pdf`;
        ...
        <>
            ...
            <div className="px-4 py-3 main_content">
                {pdfUrl ? (
                    <>
                        <div className="mt-4">
                            <h1 className="mb-3">Listes Des Activites Caisse Social :</h1>
                            <iframe
                                src={pdfUrl}
                                width="100%"
                                height="800px"
                    ...
                    <>
                        <div className="d-flex align-items-center justify-content-between">
                            <h1 className="mb-3">Liste activites</h1>
                            {cotisationPermission && cotisationPermission.CAN_WRITE ?
                                <Button
                                    className="mt-3 ml-3 button-mobile"
                                    size="small"
                                    onClick={() => {
                                        navigate("/caisseSocialliste/new");
                                ...
                                .round-indicator .p-image-preview-indicator {
                                                ...
                                                <>
                                                    ...
                                                    {item?.beneficiaire ?
                                                        <Link
                                                            ...
                                                            className=" text-decoration-none d-flex round-indicator"
                                                            style={{ color: '#399af2' }}
                                                            to={`/utilisateurs/${encodeId(item?.beneficiaire?.ID_UTILISATEUR)}`}
                                                            onClick={(e) => {
                                                                e.stopPropagation();
                                                            }}
                                                            data-pr-position="bottom"
                                    ...
                                    <Column
                                        ...
                                        body={(item) => {
                                            return (
                                                <span>
                                                    {item.typeoperation?.NOM_OPERATION ? item.typeoperation?.NOM_OPERATION : "-"}
                                                </span>
                                            );
                                        }}
                                    />
                                    ...
                                    <Column
                                        ...
                                        sortable
                                        body={(item) => {
                                            return moment(item.DATE_ENREGISTREMENT ? item.DATE_ENREGISTREMENT : "-").format("DD/MM/YYYY");
