...
export default function Types_charges_depenses_liste_page() {
    ...
    const [types_charges_depense, setTypes_charges_depense] = useState([]);
    ...
    const onSelectAllChange = (event) => {
        ...
        if (selectAll) {
            setSelectAll(true);
            setSelectedItems(types_charges_depense);
        } else {
            setSelectAll(false);
            setSelectedItems([]);
        }
    ...
    const deleteItems = async (itemsIds) => {
        try {
            ...
            const res = await fetchApi("/comptabilite/type_charges_depenses/deletetypechargesdepenses", {
                method: "POST",
                body: form,
            });
            dispacth(
                setToastAction({
                    severity: "success",
                    summary: "types chatges depense supprimé",
                    detail: "types charges depenses a été supprimé avec succès",
                    life: 3000,
                })
            );
            fetchTypes_charge_depense();
            setSelectAll(false);
    ...
    const fetchTypes_charge_depense = useCallback(async () => {
        try {
            setLoading(true);
            const baseurl = `/comptabilite/type_charges_depenses/fetchtypechargedepense?`;
            var url = baseurl;
            ...
            setTypes_charges_depense(res.result.data);
    ...
    useEffect(() => {
        document.title = " Types charges depenses"
        dispacth(setBreadCrumbItemsAction([
            {
                path: 'type_charge_depenses',
                name: 'type charge depenses'
            }
        ]));
        return () => {
            dispacth(setBreadCrumbItemsAction([]));
                                        ...
                                        const items = [

                                            {
                                                template: (deleteItem, options) => {
                                                    return (
                                                        <Link
                                                            to={`/type_charge_depenses/edit/${encodeId(inViewMenuItem?.ID_TYPES_CHARGES_DEPENSE)}`}
                                                ...
                                                template: (deleteItem, options) => {
                                                    return (
                                                        <a
                                                            ...
                                                            onClick={(e) =>
                                                                handleDeletePress(e, [
                                                                    inViewMenuItem?.ID_TYPES_CHARGES_DEPENSE,
                                                                ])
                                                            }
                                                        >
                                                            <svg
