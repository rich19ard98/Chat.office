...
export default function Penalites_Retard_edit() {
  const dispacth = useDispatch();
  const [data, handleChange, setData, setValue] = useForm(initialForm);
  const [membre, setMembres] = useState([])
  const { ID_PENALITE_RETARD: encodedId } = useParams(); // Extract ID_MEMBRE from URL params
  const ID_PENALITE_RETARD = decodeId(encodedId);
  const [loading, setLoading] = useState(true);

  const [frais, setFrais] = useState(true);
  ...
  const handleSubmit = async (e) => {
    try {
      ...
      if (isValidate()) {
        ...
        const res = await fetchApi(`/presence/Penalite_retard/update/${ID_PENALITE_RETARD}`, {
          method: "POST",
          body: form,
        });
  ...
  useEffect(() => {
    dispacth(
      setBreadCrumbItemsAction([
        ...
        administration_routes_items.Penalite_retard_edit,
      ])
    );
    return () => {
  ...
  const fetchCotisation = useCallback(async () => {
    try {
      setLoading(true);
      const baseurl = `/presence/Penalite_retard/find/${ID_PENALITE_RETARD}?`;
      ...
      const util = res.result
      setData({
        N_PENALITE_RETARD: util.N_PENALITE_RETARD == 0 ? "Retard" : "Absenet",

        MONTANT: util.MONTANT
      });
  ...
  return (
    <>
      ...
      <div className="px-4 py-3 main_content bg-white has_footer">
        <div className="">
          <h1 className="mb-3">Modification Penalite retard</h1>
