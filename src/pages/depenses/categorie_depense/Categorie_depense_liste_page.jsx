...
export default function Categorie_depense_liste_page() {
  ...
  const deleteItems = async (itemsIds) => {
    try {
      ...
      const res = await fetchApi("/depenses/categorieDepense/delete", {
        method: "POST",
        body: form,
      });
      dispacth(
        setToastAction({
          severity: "success",
          summary: "La catégorie dépense supprimé",
          detail: "La catégorie dépense  a bien a été supprimé avec succès",
          life: 3000,
        })
      );
  ...
  const fetchcategorieDepense = useCallback(async () => {
    try {
      setLoading(true);
      const baseurl = `/depenses/categorieDepense/fetch?`;
      var url = baseurl;
  ...
  useEffect(() => {
    dispacth(setBreadCrumbItemsAction([comptabilite_routes_items.categDep]));
    return () => {
          ...
          {comptabilitePermission && comptabilitePermission.CAN_WRITE ?
            <div className="selection-actions d-flex align-items-center">
              ...
              ${(!selectedItems || selectedItems?.length == 0) && 'opacity-50 pointer-events-none'}`}
                style={{}} onClick={e => handleDeletePress(e, selectedItems.map(item => item.ID_CATEGORIES_DEPENSES))}
              >
                <svg
        ...
        <div className="content">
          <div className="shadow rounded mt-3 pr-1 bg-white">
            <DataTable
              ...
              size="small"
              paginator
              rowsPerPageOptions={[5, 10, 25, 50, 150, 250, 500]}
            ...
            >
              ...
              <Column
                ...
                header="Nom"
                sortable
                body={(item) => item.NOM_DEPENSES ? item.NOM_DEPENSES : '-'}
              />
              <Column
                field="DESCRIPTION"
                header="Description"
                sortable
                body={(item) => item.DESCRIPTION ? item.DESCRIPTION : '-'}
              />
              <Column
                    ...
                    const items = [

                      {
                        template: (deleteItem, options) => {
                          return (
                            <Link
                              to={`/categDep/edit/${encodeId(inViewMenuItem?.ID_CATEGORIES_DEPENSES)}`}
                        ...
                        template: (deleteItem, options) => {
                          return (
                            <a
                              ...
                              onClick={(e) =>
                                handleDeletePress(e, [
                                  inViewMenuItem.ID_CATEGORIES_DEPENSES,
                                ])
                              }
                            >
                              <svg
