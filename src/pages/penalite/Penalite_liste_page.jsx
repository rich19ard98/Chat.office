...
export default function Penalite_liste_page() {
  ...
  const fetchPenalite = useCallback(async () => {
    try {
      setLoading(true);
      const baseurl = `/credits/penalite/fetch? `;
      var url = baseurl;
                    ...
                    const items = [

                      {
                        template: (deleteItem, options) => {
                          return (
                            <Link
                              to={`/penalite/edit/${inViewMenuItem?.ID_PENALITE}`}
