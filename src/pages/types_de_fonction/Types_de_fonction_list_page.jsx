...
export default function Types_de_fonction_list_page() {
    ...
    const [types_de_fonction, setTypes_de_fonction] = useState(null);
    ...
    const onSelectAllChange = (event) => {
        ...
        if (selectAll) {
            setSelectAll(true);
            setSelectedItems(types_de_fonction);
        } else {
            setSelectAll(false);
            setSelectedItems([]);
        }
    ...
    const deleteItems = async (itemsIds) => {
        try {
            ...
            const res = await fetchApi("/gerers/types_de_fonction/delete_fonction", {
                method: "POST",
                body: form,
            });
            dispacth(
                setToastAction({
                    severity: "success",
                    summary: "  Fonction  supprimé",
                    detail: "La fonction a été supprimé avec succès",
                    life: 3000,
                })
            );
    ...
    const fetchtypes_de_fonction = useCallback(async () => {
        try {
            setLoading(true);
            const baseurl = `/gerers/types_de_fonction/fetch?`;
            var url = baseurl;
            ...
            setTypes_de_fonction(res.result.data);
    ...
    useEffect(() => {
        document.title = "fonction"
        dispacth(setBreadCrumbItemsAction([administration_routes_items.types_de_fonction]));
        return () => {
        ...
        <>
            ...
            <div className="px-4 py-3 main_content">
                <div className="d-flex align-items-center justify-content-between">
                    <h1 className="mb-3">Types de fonction</h1>
                    {gerePermission && gerePermission.CAN_WRITE ?
                        <Button
                            ...
                            size="small"
                            onClick={() => {
                                navigate("/types_de_fonction/new");
                            }}
                        />
                        : null}
                </div>
                        ...
                        <div className="selection-actions d-flex align-items-center ml-3">
                            ...
                            <a
                                ...
                                onClick={(e) =>
                                    handleDeletePress(
                                        e,
                                        selectedItems.map((item) => item.ID_TYPE_FONCTIONS)
                                    )
                                }
                            >
                                <svg
                                        ...
                                        const items = [

                                            {
                                                template: (deleteItem, options) => {
                                                    return (
                                                        <Link
                                                            to={`/types_de_fonction/edit/${encodeId(inViewMenuItem?.ID_TYPE_FONCTIONS)}`}
                                                ...
                                                template: (deleteItem, options) => {
                                                    return (
                                                        <a
                                                            ...
                                                            onClick={(e) =>
                                                                handleDeletePress(e, [
                                                                    inViewMenuItem.ID_TYPE_FONCTIONS,
                                                                ])
                                                            }
                                                        >
                                                            <svg
