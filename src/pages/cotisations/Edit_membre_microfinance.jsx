...
export default function Edit_membre_microfinance() {
    const dispacth = useDispatch();
    const urlParams = new URLSearchParams(window.location.search);
    const [data, handleChange, setData, setValue] = useForm(initialForm);
    const [profil, setProfil] = useState([]);
    const { ID_MEMBRES_MICROFINANCE: encodedId } = useParams(); // Extract ID_MEMBRE from URL params
    const ID_MEMBRES_MICROFINANCE = decodeId(encodedId); // Decode the ID if necessary
    ...
    const {
        ...
        },
        {
            ...
            TELEPHONE: {
                required: "Ce champ est obligatoire",
                length: "Le numéro ne doit pas dépasser 20 chiffres",
                alpha: "Le téléphone peut être alphanumérique uniquement",
    ...
    const handleSubmit = async (e) => {
        try {
            ...
            if (isValidate()) {
                ...
                const res = await fetchApi(`/cotisation/membres_microfinance/update/${ID_MEMBRES_MICROFINANCE}`, {
                    method: "POST",
                    body: form,
    ...
    useEffect(() => {
        (async () => {
            try {
                const res = await fetchApi(`/cotisation/membres_microfinance/find/${ID_MEMBRES_MICROFINANCE}`);
                if (res && res.result) {
                    const utilisateur = res.result;
                    // Si tu veux les données du membre lié (via la jointure membre)
                    const membre = utilisateur.membre || utilisateur; // fallback si membre non disponible
                    ...
                    setData({
                        ...
                        DATE_NAISSANCE: membre?.DATE_NAISSANCE ? new Date(membre.DATE_NAISSANCE) : null,
                    ...
                    console.error('Aucun utilisateur trouvé ou réponse invalide', res);
                }
            } catch (error) {
    ...
    useEffect(() => {
        document.title = "Edit Membre"
        dispacth(
            setBreadCrumbItemsAction([
                administration_routes_items.membre_microfinance,
                administration_routes_items.Edit_membre_microfinance,
            ])
        );
        return () => {
