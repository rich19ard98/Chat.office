...
export default function Cotisation_liste_page() {
    ...
    const [soldeMap, setSoldeMap] = useState({});
    ...
    const [typecotisations, settypecotisations] = useState(false)
    ...
    const { ID_COTISATIONS: encodedId } = useParams();
    const ID_COTISATIONS = decodeId(encodedId); // Décoder pour obtenir l'ID réel
    ...
    const setcotisation = (type) => {
        settypecotisations(type);
    };
    ...
    // 📌 Fonction qui recharge depuis l’API (pour WS / autres clients)
    const fetchCotisation = useCallback(async (silent = false) => {
        try {
            if (!silent) setLoading(true); // 🔹 seulement si pas silencieux
            ...
            if (ID_COTISATIONS) {
                url += `ID_COTISATIONS=${ID_COTISATIONS}&`;
            }
            if (typecotisations) {
                url += `typecotisations=${typecotisations.code}&`
            }

            const res = await fetchApi(url);
            const membresAvecSolde = res.result.Solde
            ...
            // Maintenant on peut calculer NOMBRE_ACTIONS pour chaque cotisation
            setFrais(
                res.result.data
            )
            ...
            if (!silent) setLoading(false); // 🔹 ne stoppe le loading que si ce n’était pas silencieux
        }
    }, [lazyState, startDate, typecotisations, endDate, dates, ID_COTISATIONS, selectedMembre]);

    useEffect(() => {
        fetchCotisation();
    }, [lazyState, startDate, typecotisations, endDate, dates, ID_COTISATIONS, selectedMembre]);

    const fetchRef = useRef((silent) => { }); // init

    useEffect(() => {
        // on garde toujours la version à jour de fetchCotisation
        fetchRef.current = (silent = false) => fetchCotisation(silent);
    }, [fetchCotisation]);

    useEffect(() => {
        const handler = (payload) => {

            if (["cotisation", "cotisationAnticipe"].includes(payload.type)) {
                if (["ajout", "update", "delete"].includes(payload.action)) {
                    fetchRef.current(true); // ⚡ recharge en arrière-plan (silent = true)
                }
            }
        };

        socket.on("new_data", handler);
        return () => socket.off("new_data", handler);
    }, []);
                ...
                {pdfUrl ? (
                    ...
                    <>
                        <div className="d-flex align-items-center justify-content-between">
                            <h1 className="mb-3">Cotisation</h1>
                            {isAdmin !== 1
                        ...
                        <div className="shadow my-2 bg-white p-3 rounded d-flex align-items-center justify-content-between">
                            <div className="d-flex  align-items-center">
                                ...
                                <div className="d-flex flex-column mx-1">
                                    <Dropdown
                                        value={typecotisations}
                                        onChange={(e) => { setcotisation(e.value); }}
                                        options={typecotisation}
                                        filter
                                        filterBy="name"
                                        optionLabel="name"
                                        placeholder="Type cotisation"
                                        className="w-full md:w-10rem mx-3 no-p"
                                        showClear
                                        style={{ minWidth: 300 }}
                                    />
                            ...
                            <div className="shadow rounded mt-3 pr-1 bg-white" style={{ paddingBottom: '4rem' }}>
                                ...
                                .round-indicator .p-image-preview-indicator {
                                    ...
                                    <Column
                                        ...
                                        body={(item) => {
                                            return (
                                                <span>{item?.REFERENCE_COTISATION || "-"}</span>
                                            );
                                        }}
                                    />
                                    ...
                                    <Column
                                        field="MONTANT"
                                        header="Montant C.Retard"
                                        sortable
                                        body={(item) => {
                                            return (
                                                <span>{item.INTERET_COTISATION_TARDIVE ? parseInt(item.INTERET_COTISATION_TARDIVE).toLocaleString('fr-FR') : 0} Fbu</span>
                                            );
                                        }}
                                    />
